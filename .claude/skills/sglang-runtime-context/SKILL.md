...
|------|----------|-------|-----------|
| raw config seed | `get_server_args()` | the published **pristine** `ServerArgs` (resolved-at-startup record; kept for debugging, dumps, per-runner fork copies) | published once per process; read-only |
...
- Every process entry point calls `publish(server_args, role=...)`
  (`run_scheduler_process`, the Ray `SchedulerActor`, the DP controller, tokenizer,
  encoder, weight-cache daemon, launcher, ...). `publish` snapshots the resolved field
  values into the config bags; the accessors (`get_exec()` etc.) fail closed before it
  runs. `role` records which process type published (provenance today; per-role
  namespace enforcement is planned).
...
- **Per-instance boundaries** — the tokenizer-manager family, everything under
  ...
  bags are last-publish-wins across engines.
...
those must stay on `server_args.X` at call sites that run unconditionally —
conditionally-initialized groups (DCP / attn-CP / moe-DP) fail loud when the group
does not exist. `ParallelContext.__getattr__` is deliberately dynamo-traceable (no
...
- Values only knowable at weight-load time → `declare_load_time_override(source, {...})`
  ...
  Draft-model declarations land on the draft's bags inside the build's
  `preserve_config` scope and are discarded with it.
...
1. **Strict mutation guard** (`SGLANG_STRICT_CONFIG_MUTATION=1`, default-on in tests):
   bare `server_args.x = ...` after resolution raises; projected bags are sealed the same
   way (leaf assignment raises — write via `get_context().override`).
...
- **Moving code drops first-line guards**: early returns (`if self.is_draft_worker: return`)
  are the easiest thing to lose when relocating a method body. Draft workers run against
  their own config copy/bags — a draft-side write against the target's state poisons it.
